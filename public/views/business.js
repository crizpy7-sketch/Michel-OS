...
import { card, chip, denied, empty, field, input, select, sheet, toast, withStates } from '../lib/ui.js';
...
function overview(base) {
  ...
  const employees = allEmployees.filter((employee) => employee.active !== false);
  ...
  const employeeSection = employees.length ? h('section', { class: 'section business-section' },
    ...
    ...employees.slice(0, 8).map((employee) => h('a', {
      class: 'business-list-row business-list-row--link', href: '/business/staffing',
  ...
  const shiftSection = shifts.length ? h('section', { class: 'section business-section' },
    ...
    ...shifts.slice(0, 8).map((shift) => {
      const person = shift.employeeId ? allEmployees.find((employee) => employee.id === shift.employeeId)?.displayName ?? 'Unknown employee' : 'Unassigned';
...
function staffing(base, extra, mount) {
  const allEmployees = base.employees ?? [];
  const employees = allEmployees.filter((employee) => employee.active !== false);
  const formerEmployees = allEmployees.filter((employee) => employee.active === false);
  const employeeById = new Map(allEmployees.map((e) => [e.id, e]));
  const availability = extra.availability ?? [];
  const timeOff = extra.timeOff ?? [];
  return h('div', { class: 'business-staffing' },
    employeeRoster(employees, formerEmployees, base, mount),
    ...
    (base.shifts ?? []).length === 0 ? empty({ title: 'No shifts scheduled', body: 'Assign employee work schedules here so coverage is visible alongside family commitments.' })
      : h('section', { class: 'business-shift-list', 'aria-label': 'Scheduled shifts' }, ...(base.shifts ?? []).map((shift) => {
...
function employeeRoster(employees, formerEmployees, base, mount) {
  const active = h('section', { class: 'section business-section' },
    h('div', { class: 'section__head' }, h('h2', { class: 'section__title' }, 'Active staff'), chip(plural(employees.length, 'employee'))),
    employees.length === 0 ? h('p', { class: 'muted' }, 'No active employees yet.')
      : employees.map((employee) => employeeRow(employee, base, mount, true)),
  );
  const former = formerEmployees.length === 0 ? null : h('section', { class: 'section business-section business-former-staff' },
    h('div', { class: 'section__head' }, h('h2', { class: 'section__title' }, 'Former staff'), chip(plural(formerEmployees.length, 'employee'), 'quiet')),
    h('p', { class: 'muted tiny' }, 'Kept here so past schedules and business history stay accurate.'),
    ...formerEmployees.map((employee) => employeeRow(employee, base, mount, false)),
  );
  return h('div', { class: 'business-roster' }, active, former);
}

function employeeRow(employee, base, mount, active) {
  const action = state.can('business.manage') ? h('button', {
    class: `btn btn--small ${active ? 'btn--danger' : ''}`.trim(), type: 'button',
    onClick: () => employeeStatusSheet(employee, active, mount),
  }, active ? 'Remove' : 'Restore') : null;
  return h('div', { class: `business-list-row business-employee-row${active ? '' : ' business-employee-row--inactive'}` },
    h('span', { class: 'business-avatar', 'aria-hidden': 'true' }, initials(employee.displayName)),
    h('div', { class: 'business-list-row__main' },
      h('strong', {}, employee.displayName),
      h('span', { class: 'muted tiny' }, active
        ? `${scheduledHours(base, employee.id) ?? 0} scheduled hours`
        : 'Not available for new shifts'),
    ), action,
  );
}

function employeeStatusSheet(employee, active, mount) {
  sheet(active ? 'Remove from active staff' : 'Restore employee', (close) => {
    const verb = active ? 'Remove from active staff' : 'Restore to active staff';
    const body = active
      ? `${employee.displayName} will disappear from employee pickers and new schedules. Past shifts and business history will stay.`
      : `${employee.displayName} will be available in employee pickers and new schedules again.`;
    return h('form', { class: 'employee-status-form', onSubmit: (event) => submitMutation(event,
      active ? 'Removing…' : 'Restoring…', async () => {
        await api.patch(`/api/households/${state.household.id}/business/employees/${employee.id}`, { active: !active });
        close();
        toast(active ? `${employee.displayName} removed from active staff` : `${employee.displayName} restored`);
        await load(mount, 'staffing');
      }, active ? 'Could not remove this employee.' : 'Could not restore this employee.') },
      h('p', { class: 'employee-status-form__body' }, body),
      h('div', { class: 'employee-status-form__actions' },
        h('button', { class: 'btn', type: 'button', onClick: close }, 'Cancel'),
        h('button', { class: `btn ${active ? 'btn--danger' : 'btn--primary'}`, type: 'submit' }, verb),
      ),
    );
  });
}
...
function addEmployee(mount) {
  ...
  return card('Add employee', null, h('form', { onSubmit: (e) => submitMutation(e, 'Adding…', async () => {
  ...
  }, 'Could not add employee.') }, field('Name', name), field('Hourly rate', rate), h('button', { class: 'btn btn--primary', type: 'submit' }, 'Add employee')));
}
...
function addAvailability(employees, mount) {
  ...
  return card('Employee availability', null, h('form', { onSubmit: (e) => submitMutation(e, 'Saving…', async () => {
...
function addTimeOff(employees, mount) {
  ...
  return card('Time off', null, h('form', { onSubmit: (e) => submitMutation(e, 'Saving…', async () => {
  ...
  }, 'Could not save time off.') }, field('Employee', employee), h('div', { class: 'split' }, field('Starts', starts), field('Ends', ends)), field('Reason', reason), h('button', { class: 'btn', type: 'submit' }, 'Add time off')));
}

function addShift(base, mount) {
  const employee = select([['', 'Unassigned'], ...(base.employees ?? []).filter((item) => item.active !== false).map((e) => [e.id, e.displayName])]);
  ...
  return card('Assign a shift', null, h('form', { onSubmit: (e) => submitMutation(e, 'Assigning…', async () => {
    ...
    const warnings = result.warnings ?? []; toast(warnings.length ? `Shift saved with ${warnings.length} warning${warnings.length === 1 ? '' : 's'}` : 'Shift saved', warnings.length ? 'warn' : 'good'); await load(mount, 'staffing');
  }, 'Could not assign shift.') }, field('Employee', employee), h('div', { class: 'split' }, field('Starts', starts), field('Ends', ends)), field('Role', role), h('button', { class: 'btn btn--primary', type: 'submit' }, 'Assign shift')));
}
...
function publishBox(mount) {
  return card('Publish schedule', null, h('p', { style: { color: 'var(--muted)' } }, 'Publishing runs the staffing rules again. Blocking conflicts are refused instead of silently changing the schedule.'),
    h('form', { onSubmit: (e) => submitMutation(e, 'Publishing…', async () => {
      const result = await api.post(`/api/households/${state.household.id}/business/publish`, {});
      const warnings = result.warnings ?? [];
      toast(`${result.published ?? 0} shift(s) published${warnings.length ? ` with ${warnings.length} warning${warnings.length === 1 ? '' : 's'}` : ''}`, warnings.length ? 'warn' : 'good');
      await load(mount, 'staffing');
    }, 'Schedule could not be published.') },
    h('button', { class: 'btn', type: 'submit' }, 'Publish current schedule')));
}
...
function addProduct(mount) {
  ...
  return card('Add product', null, h('form', { onSubmit: (e) => submitMutation(e, 'Adding…', async () => {
  ...
  }, 'Could not add product.') }, field('SKU', sku), field('Product', name), h('div', { class: 'split' }, field('Starting quantity', qty), field('Reorder point', reorder)), h('div', { class: 'split' }, field('Price', price), field('Cost', cost)), h('button', { class: 'btn btn--primary', type: 'submit' }, 'Add product')));
}
...
function adjustStock(products, mount) {
  ...
  return card('Adjust stock', null, h('form', { onSubmit: (e) => submitMutation(e, 'Updating…', async () => {
  ...
  }, 'Could not update inventory.') }, field('Product', product), field('Movement', kind), field('Quantity change', delta, { hint: 'Receive and return require a positive number. Shrinkage requires a negative number. Adjustment can go either direction.' }), h('button', { class: 'btn', type: 'submit' }, 'Update stock')));
}
...
function recordExpenseBox(mount) {
  const vendor = input({ required: true, placeholder: 'Vendor' }); const category = input({ required: true, placeholder: 'Supplies, rent, shipping…' }); const amount = input({ required: true, type: 'number', min: 0.01, step: 0.01 });
  return card('Record expense', null, h('form', { onSubmit: (e) => submitMutation(e, 'Recording…', async () => { await api.post(`/api/households/${state.household.id}/business/expenses`, { vendor: vendor.value.trim(), category: category.value.trim(), amountCents: Math.round(Number(amount.value) * 100) }); toast('Expense recorded'); await load(mount, 'finance'); }, 'Could not record expense.') }, field('Vendor', vendor), field('Category', category), field('Amount', amount), h('button', { class: 'btn', type: 'submit' }, 'Record expense')));
}
...
function recordSaleBox(products, mount) {
  ...
  return card('Record sale', null, h('form', { onSubmit: (e) => submitMutation(e, 'Recording…', async () => { const picked = products.find((p) => p.id === product.value); await api.post(`/api/households/${state.household.id}/business/sales`, { items: [{ productId: picked.id, quantity: Number(qty.value), unitPriceCents: picked.unitPrice ?? 0 }] }); toast('Sale recorded'); await load(mount, 'finance'); }, 'Could not record sale.') }, field('Product', product), field('Quantity', qty), h('button', { class: 'btn btn--primary', type: 'submit' }, 'Record sale')));
}
...
function createBusiness(mount) {
  ...
  return card('Set up Shia Baby', null, h('form', { onSubmit: (e) => submitMutation(e, 'Creating…', async () => { await api.post(`/api/households/${state.household.id}/business`, { name: name.value.trim(), timezone: state.timezone, taxSetAsideRate: Number(rate.value) / 100 }); state.business = { name: name.value.trim() }; toast('Business workspace created'); await load(mount, 'overview'); }, 'Could not create business.') }, field('Business name', name), field('Tax set-aside percentage', rate, { hint: 'An estimate only. Michel OS always shows the disclaimer with the number.' }), h('button', { class: 'btn btn--primary', type: 'submit' }, 'Create workspace')));
}

async function submitMutation(event, pendingLabel, action, fallbackMessage) {
  ...
  const oldError = form.querySelector('.form-status--error');
  oldError?.remove();
  ...
  try {
    ...
    form.append(h('p', { class: 'form-status form-status--error', role: 'alert', tabindex: '-1' }, `${message} Nothing was changed. Try again.`));
    form.querySelector('.form-status--error')?.focus();
    toast(message, 'error');
  } finally {
    if (form.isConnected) {
