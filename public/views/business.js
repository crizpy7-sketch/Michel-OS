...
async function load(mount, sectionName) {
  await withStates(mount, sectionName === 'finance' ? 'dash' : 'list', async () => {
    ...
    const extra = sectionName === 'inventory' ? await api.get(`/api/households/${state.household.id}/business/inventory`)
      : sectionName === 'finance' ? await api.get(`/api/households/${state.household.id}/business/finance?period=week`) : null;
    return { base, extra };
  }, ({ base, extra }) => h('div', {}, nav(sectionName), sectionName === 'staffing' ? staffing(base, mount)
...
function addShift(base, mount) {
  ...
  } catch (error) { toast(error.message ?? 'Could not assign shift.', 'error'); } } },
...
function addProduct(mount) {
  const sku = input({ required: true, placeholder: 'SKU-001' }); const name = input({ required: true, placeholder: 'Product name' });
  const qty = input({ type: 'number', value: 0, min: 0, step: 1 }); const reorder = input({ type: 'number', value: 0, min: 0, step: 1 });
  const price = input({ type: 'number', min: 0, step: 0.01, placeholder: '0.00' }); const cost = input({ type: 'number', min: 0, step: 0.01, placeholder: '0.00' });
  return card('Add product', null, h('form', { onSubmit: async (e) => { e.preventDefault(); try {
    await api.post(`/api/households/${state.household.id}/business/products`, { sku: sku.value.trim(), name: name.value.trim(), quantityOnHand: Number(qty.value), reorderPoint: Number(reorder.value), unitPriceCents: Math.round(Number(price.value || 0) * 100), unitCostCents: Math.round(Number(cost.value || 0) * 100) });
    toast('Product added'); await load(mount, 'inventory');
...
function recordExpenseBox(mount) {
  ...
  return card('Record expense', null, h('form', { onSubmit: async (e) => { e.preventDefault(); try {
    await api.post(`/api/households/${state.household.id}/business/expenses`, { vendor: vendor.value.trim(), category: category.value.trim(), amountCents: Math.round(Number(amount.value) * 100) });
    toast('Expense recorded'); await load(mount, 'finance');
  } catch (error) { toast(error.message ?? 'Could not record expense.', 'error'); } } }, field('Vendor', vendor), field('Category', category), field('Amount', amount), h('button', { class: 'btn', type: 'submit' }, 'Record expense')));
}
...
function createBusiness(mount) {
  if (!state.can('household.manage')) return empty({ title: 'Shia Baby is not set up', body: 'A household owner can create the business workspace.' });
  const name = input({ value: 'Shia Baby', required: true }); const rate = input({ type: 'number', min: 0, max: 100, step: 0.1, value: 0, required: true });
  return card('Set up Shia Baby', null, h('form', { onSubmit: async (e) => { e.preventDefault(); try {
    await api.post(`/api/households/${state.household.id}/business`, { name: name.value.trim(), timezone: state.timezone, taxSetAsideRate: Number(rate.value) / 100 });
    state.business = { name: name.value.trim() }; toast('Business workspace created'); await load(mount, 'overview');
  } catch (error) { toast(error.message ?? 'Could not create business.', 'error'); } } }, field('Business name', name), field('Tax set-aside percentage', rate, { hint: 'An estimate only. Michel OS always shows the disclaimer with the number.' }), h('button', { class: 'btn btn--primary', type: 'submit' }, 'Create workspace')));
}
